-- Complete Image Cleanup and Fix Script
-- This script:
-- 1. Removes ALL existing images (both placeholder and real)
-- 2. Adds only working Unsplash images
-- Run with: psql -U postgres -d amazon_clone -f CLEANUP_AND_FIX_IMAGES.sql

-- Step 1: Remove ALL existing product images
DELETE FROM product_images;
SELECT 'All existing images deleted' as status;

-- Step 2: Add working Unsplash images for all products
-- These are tested working image URLs
...
-- Step 3: Verify the cleanup and additions
SELECT 'CLEANUP AND FIX COMPLETE!' as status;

SELECT 
    'Total images added: ' || COUNT(*) as result
FROM product_images;

-- Show final image summary for each product
SELECT 
    p.id,
